...
-(void)initSet{
    ...
    self.req_freq = @"d"; // 股票代码 规则是：沪股代码末尾加.ss，深股代码末尾加.sz。如浦发银行的代号是：600000.SS
   // self.req_url = kMProductKlineDayURL;
...
-(void)drawBox{
    ...
    if (!isUpdate) {
        ...
        for (int i = 0; i<7; i++) {
            ...
            CMLines *line = [[CMLines alloc] initWithFrame:CGRectMake(0, 0, mainboxView.frame.size.width, mainboxView.frame.size.height)];
            line.color = @"#333333";
...
-(void)drawLine{
    ...
    if (lineOldArray.count>0 && isUpdate) {
        for (CMLines *line in lineOldArray) {
...
-(void)drawBoxWithKline{
    ...
    CMLines *kline = [[CMLines alloc] initWithFrame:CGRectMake(0, 0, mainboxView.frame.size.width, mainboxView.frame.size.height)];
    ...
    CMLines *volline = [[CMLines alloc] initWithFrame:CGRectMake(0, 0, bottomBoxView.frame.size.width, bottomBoxView.frame.size.height)];
    volline.points = voltempArray;
    volline.lineWidth = self.kLineWidth;
    volline.isK = YES;
    volline.isVol = YES;
    [bottomBoxView addSubview:volline];
    volMaxValueLab.text = [CMCommonTool changePrice:getdata.volMaxValue];
