...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        _TitleDict=@{@"如何成为注册用户?":@[@"在注册页面,填写您真实的邮箱或手机号、密码并同意相关协议,点击‘注册'按钮即可完成注册。"],
                     @"什么是倍利宝？":@[@"倍利宝是一种创新型的投资产品,主要有三种产品组合,一种是一级市场众筹计划的组合,一种是二级市场综合投资的组合,还有一种是一级市场和二级市场资金错配进行组合的产品。可灵活赎回,不可进入二级市场进行交易。"],
    ...
    switch (sender.tag) {
        ...
        case 13:
        {
            _TitleDict=@{@"倍利宝产品的收益是如何分配的？":@[@"1、收益分配方式:红利续投,会体现在净值当中",@"2、每一类别倍利宝份额享有同等分配权"],
                         ...
                         @"收益=赎回时总金额-买入成本":@[@"举例:假如您2016年4月30号申购倍利宝1号10000份,当日净值100,2016年10月30日赎回。当日净值:110 、赎回费率:1% 、 持有份数:10000赎回后获得现金为:10000*110*（1-1%）==1089000 对比本金1000000元,投资收益为:89000"]
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return rect.size.height + 13 ;
}
-(UITableView*)currTablview{
    if (!_currTablview) {
        _currTablview=[[UITableView alloc]initWithFrame:CGRectMake(0,60, CMScreen_width(), f_i5real(300)) style:UITableViewStylePlain];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if([scrollView isKindOfClass:[self.currTablview class]]){
