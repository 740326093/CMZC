...
- (void)_setup {
    ...
    self.minMALineColor = [UIColor clmHex:0x019ffd];
    self.midMALineColor = [UIColor clmHex:0xff9900];
    self.maxMALineColor = [UIColor clmHex:0xff00ff];
    
    self.positiveVolColor = self.positiveLineColor;
    self.negativeVolColor =  self.negativeLineColor;
    
    self.axisShadowColor = [UIColor clmHex:0x333333];
    self.axisShadowWidth = 0.5;
    
    self.separatorColor =[UIColor clmHex:0x333333];
    ...
    self.crossLineColor = [UIColor clmHex:0xc9c9c9];
...
- (void)longPressEvent:(UILongPressGestureRecognizer *)longGesture {
    ...
    if (longGesture.state == UIGestureRecognizerStateEnded) {
        ...
        [self.xAxisContext enumerateKeysAndObjectsUsingBlock:^(NSNumber *xAxisKey, NSNumber *indexObject, BOOL *stop) {
            //&& ([xAxisKey floatValue] - touchPoint.x) > 0
           if (_kLinePadding+_kLineWidth >= ([xAxisKey floatValue] - touchPoint.x)&&self.yAxisHeight>touchPoint.y) {
                ...
                [self configUIWithLine:line atPoint:CGPointMake(touchPoint.x, touchPoint.y)];
...
- (void)configUIWithLine:(NSArray *)line atPoint:(CGPoint)point {
    ...
    //提示版
    self.tipBoard.open = [self decimalProcessingWithNum:[line[0] stringValue]];
    self.tipBoard.close = [self decimalProcessingWithNum:[line[3] stringValue]];
    self.tipBoard.high = [self decimalProcessingWithNum:[line[1] stringValue]];
    self.tipBoard.low = [self decimalProcessingWithNum:[line[2] stringValue]];
...
- (void)drawAxisInRect:(CGRect)rect {
    ...
    for (int i = 1; i <= 4; i ++) {
        ...
        CGContextAddLineToPoint(context, rect.size.width  - self.rightMargin - 0.5, self.topMargin + avgHeight*i);
    ...
    for (int i = 0; i < 6; i ++) {
        ...
        //MyLog(@"yAxisValue+++++%.2f",yAxisValue);
...
- (void)drawBottomLeftLabelrect:(CGRect)rect{
    ...
    NSString     *tBottomyAxisTitle;
    NSString    *bBottomyAxisTitle;
    
    if (self.showBarChart) {
      //  if (self.maxVolValue<10000.0) {
            tBottomyAxisTitle  = [self roundFloatDisplay:self.maxVolValue];
       ...
       // }
        ...
        tBottomyAxisTitle  = [NSString stringWithFormat:@"%.2f",self.maxKDJValue];
        bBottomyAxisTitle = @"0";
    ...
    NSAttributedString *attString = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@",tBottomyAxisTitle] attributes:@{NSFontAttributeName:self.yAxisTitleFont, NSForegroundColorAttributeName:self.yAxisTitleColor}];
    CGSize size = [attString boundingRectWithSize:CGSizeMake(self.leftMargin, self.yAxisTitleFont.lineHeight) options:NSStringDrawingUsesLineFragmentOrigin context:nil].size;
    
    [attString drawInRect:CGRectMake(self.leftMargin - size.width-4, self.yAxisHeight + self.topMargin + self.timeAxisHeight - 2, size.width+10, size.height)];
 ...
 */
- (void)drawMACD {
    NSArray<UIColor *> *colors = @[[UIColor clmHex:0x67f0f1], [UIColor clmHex:0xde62f7], [UIColor clmHex:0xe2823c]];
...
- (CGPathRef)movingMACDGraphPathForContextAtIndex:(NSInteger)index macdIndex:(NSInteger)macdIndex CGContextRef:(CGContextRef)context {
    ...
    if (scale !=0) {
        for (NSArray *line in [_contexts subarrayWithRange:NSMakeRange(self.startDrawIndex, self.kLineDrawNum)]) {
            ...
            //竖线
            CGContextSetStrokeColorWithColor(context, [UIColor clmHex:0xe43337].CGColor);
            CGContextSetLineWidth(context, 0.5);
            CGContextBeginPath(context);
 ...
 */
- (void)drawKDJ {
    NSArray<UIColor *> *colors = @[[UIColor clmHex:0xe2823c], [UIColor clmHex:0x0ae8ea], [UIColor clmHex:0xde62f7]];
...
- (void)drawRSI {
    ...
    NSArray<UIColor *> *colors = @[[UIColor clmHex:0xe2823c], [UIColor clmHex:0x4e9548], [UIColor clmHex:0x0ae8ea]];
...
- (void)drawAvgLine {
    ...
    NSArray<UIColor *> *colors = @[[UIColor clmHex:0x007aff], [UIColor clmHex:0xdc70fd],[UIColor clmHex:0xf6a921]];
...
- (KLineTipBoardView *)tipBoard {
    if (!_tipBoard) {
        _tipBoard = [[KLineTipBoardView alloc] initWithFrame:CGRectMake(self.leftMargin, self.topMargin, 90.f, 110.f)];
        
        _tipBoard.backgroundColor = [UIColor clmHex:0x1e1e1e];
...
- (UIButton *)realDataTipBtn {
    if (!_realDataTipBtn) {
        ...
        [_realDataTipBtn setTitleColor:[UIColor clmHex:0xe43337] forState:UIControlStateNormal];
        ...
        _realDataTipBtn.layer.borderColor = [UIColor clmHex:0xe43337].CGColor;
