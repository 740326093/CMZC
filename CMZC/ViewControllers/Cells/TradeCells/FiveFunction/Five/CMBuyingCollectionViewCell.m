...
@property (weak, nonatomic) IBOutlet CMFilletButton *buyBtn;
...
- (BOOL)calculateThePrice {
    ...
    if (price > _totalAssets) {
       // [self showHubView:self messageStr:@"买入总价格不得大于可用金额" time:3];
        [self showHubView:self messageStr:@"账户余额不足，请先去银证转账!" time:3];
        return NO;
    } else {
        return YES;
    }
          ...
          return;
            ...
            if ([_numberTextField.text integerValue] == 0) {
             //   _numberTextField.text = @"";
              ...
              // _numberTextField.text = @"";
                [self showHubView:self messageStr:@"买入数量必须为100的倍数" time:2];
                _numberTextField.text = @"";
                return;
               
            }else if ([_numberTextField.text integerValue]>100000) {
             // _numberTextField.text = @"";
                [self showHubView:self messageStr:@"买入数量最大不能超过10万" time:2];
                _numberTextField.text = @"";
